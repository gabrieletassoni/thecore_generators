...
class Thecore::Generators::ModelGeneratorTest < Rails::Generators::TestCase
  ...
  test "rails generate model Foo from a host-app destination_root places files in the app, with default concerns and a real test file, but no Endpoints" do
  ...
  test "cwd inside an ATOM directory places the model, migration, concerns and test file inside that ATOM" do
    build_atom_fixture!

    Dir.chdir(atom_dir) { run_generator ["Bar", "name:string"] }

    assert_file File.join(atom_dir, "app/models/bar.rb") do |content|
      assert_match(/include Api::Bar/, content)
      assert_match(/include RailsAdmin::Bar/, content)
    end
    assert_file File.join(atom_dir, "app/models/concerns/api/bar.rb")
    assert_file File.join(atom_dir, "app/models/concerns/rails_admin/bar.rb")
